...
void setClock() {
  ...
  updateStrand(strand1, HOUR_HAND_WIDTH, STRAND_1_HOUR_HAND_OFFSET,
               STRAND_1_HOURS_PERIOD * secondsInCurrentHour() * 1000, STRAND_1_FRACTIONAL_PIXELS,
  ...
  updateStrand(strand2, HOUR_HAND_WIDTH, STRAND_2_HOUR_HAND_OFFSET,
               STRAND_2_HOURS_PERIOD * secondsInCurrentHour() * 1000, STRAND_2_FRACTIONAL_PIXELS,
               hour(displayTime) * secondsInCurrentHour() * 1000 + millisSinceStartOfHour, CRGB::Red);
  updateStrand(strand3, HOUR_HAND_WIDTH, STRAND_3_HOUR_HAND_OFFSET,
               STRAND_3_HOURS_PERIOD * secondsInCurrentHour() * 1000, STRAND_3_FRACTIONAL_PIXELS,
               hour(displayTime) * secondsInCurrentHour() * 1000 + millisSinceStartOfHour, CRGB::Red);
  updateStrand(strand4, HOUR_HAND_WIDTH, STRAND_4_HOUR_HAND_OFFSET,
               STRAND_4_HOURS_PERIOD * secondsInCurrentHour() * 1000, STRAND_4_FRACTIONAL_PIXELS,
               hour(displayTime) * secondsInCurrentHour() * 1000 + millisSinceStartOfHour, CRGB::Red);
  ...
  updateStrand(strand1, MINUTE_HAND_WIDTH, STRAND_2_MINUTE_HAND_OFFSET,
               STRAND_2_MINUTES_PERIOD * secondsInMinute(displayTime) * 1000, STRAND_2_FRACTIONAL_PIXELS,
               millisSinceStartOfHour, CRGB::Blue);
  updateStrand(strand1, MINUTE_HAND_WIDTH, STRAND_3_MINUTE_HAND_OFFSET,
               STRAND_3_MINUTES_PERIOD * secondsInMinute(displayTime) * 1000, STRAND_3_FRACTIONAL_PIXELS,
               millisSinceStartOfHour, CRGB::Blue);
  updateStrand(strand1, MINUTE_HAND_WIDTH, STRAND_4_MINUTE_HAND_OFFSET,
...
void loop() {
  ...
  if (startTimeMillis - lastRefreshMillis > REFRESH_INTERVAL_MILLIS) {
    if (gps.time.isValid()) {
      Serial.write("Running setClock.\n");
      setClock();
    } else if (startTimeMillis % 1024 < 512) {
      Serial.write("Running set12oClock.\n");
      set12oClock();
    }

#ifdef DEBUG_REFRESH
    strand1[debugRefresh] = CRGB(16, 16, 16);
#endif
...
#ifdef DEBUG_REFRESH
    strand1[debugRefresh] = CRGB::Black;
